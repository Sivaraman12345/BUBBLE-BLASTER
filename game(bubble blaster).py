...
def game_intro(background):
    ...
    class IntroBubble(pygame.sprite.Sprite):
        def __init__(self, image_file, x, y, speed):
            ...
            self.speed = speed
            self.image = pygame.image.load('bubble4.png').convert_alpha()
    ...
    while intro:
        ...
        if len(introbubble_list) < intromax_bubbles:
            for i in range(random.randint(1,3)):
                IntroBubble("bubble4.png", random.randint(0, 680), 710, random.randint(1,4))
       
        # Renders text and subtext       
        text = font.render("BUBBLE BLASTER!", True, (255, 255, 255), None)
        text_rect = text.get_rect()
        subtext = smallfont.render("Click on the bubbles to BLAST them before they reach the top of your screen.", True, (255, 255, 255), None)
        subtext_rect = subtext.get_rect()
        instructions = smallfont.render("Press any key to POP.", True, (255, 255, 255), None)
        ...
        def update(self, score):
            ...
            # Allows bubbles to float from the bottom to the top of the screen based on specified speed
                ...
                if self.rect.collidepoint(*mouse_pos) and mouse_clicked[0]:
                    pop = pygame.image.load("bubble4.png").convert_alpha()
    ...
    while not close_window:
        for event in pygame.event.get():
            ...
            if event.type == bubble_drop:
                for i in range(random.randint(1,3)):
                    Bubble("bubble4.png", random.randint(0, 670), 710, random.randint(1,4))
        ...
        # If game isn't over, render background and .
            allSprites.clear(screen, background)
            allSprites.update(score)
